...
- **把动态壁纸渲染为 GUI 背景**
  ...
  - `image` 图片壁纸直接显示
  - `scene` 场景壁纸在「动态优先」模式下首次由 Wallpaper Engine **真全屏**
    渲染（无边框、置顶、按显示器物理分辨率、录制期间隐藏任务栏），
    再通过 Windows Graphics Capture 硬件编码为 12 秒的原生分辨率 H.264 循环；
    之后直接走缓存，完全避开 160px 预览图放大
