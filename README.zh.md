...
- **把动态壁纸渲染为 GUI 背景**：
  ...
  - `image` 图片壁纸直接显示，
  - `scene` 场景 / `application` 程序等壁纸渲染**从 scene.pkg 中解码出的原始背景
    贴图**——无依赖的 WE 包格式解码器（TEXV/TEXB 容器、LZ4 mipmap、DXT1/3/5、
    内嵌 PNG/JPEG；见 scripts/survey-extract.mjs——40 个采样场景 98% 提取成功，
    大量 4K 以上原图）。提取结果缓存在 `~/.dsh/we-wallpaper-cache/`；本地小动图
    作为加载中/回退层，「场景壁纸动图优先」开关可恢复纯动图模式。
...
- 场景背景是**静态图**：取 scene.pkg 中最大的横版贴图（贴图格式可解码，见
  scripts/survey-extract.mjs，但场景本体无法在浏览器中重新渲染——粒子/着色器
  效果会丢失）。极老版本的包与少量特殊贴图会回退到本地小动图（此时
  「清晰度增强」尽量补救）；「场景壁纸动图优先」开关可恢复纯动图模式。
