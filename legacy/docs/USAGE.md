...
| 场景 / Scene | **extracted background artwork from `scene.pkg`** (4K+ on most scenes), tiny GIF as the loading/fallback layer |
...
| 场景壁纸动图优先 / Animated scene previews | on = the animated (tiny) local GIF instead of the sharp extracted artwork |
...
: extraction failed for that package (very old versions / exotic textures)
or the GIF-only mode is on. Try the *animated scene previews* toggle, and
check `~/.dsh/we-wallpaper-cache/` — no file means extraction returned
nothing for that wallpaper.
